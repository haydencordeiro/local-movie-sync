...
function appendData(roomName, roomCode){
    append({name: "Party", content: "Party allows you to watch local videos with your friends synchronously while chatting.", pfp: "#f3dfbf"})
    append({name: "Party", content: `Welcome to ${roomName}`, pfp: "#f3dfbf"})
    append({name: "Party", content: `Share the room code (${roomCode}) with others to invite them to the party.`, pfp: "#f3dfbf"})
    append({name: "Party", content: "They would need to have the same video file with them to join this watch party.", pfp: "#f3dfbf"})
    // append({name: "Party", content: "You can change your username in the settings page.", pfp: "#f3dfbf"})
}
...
document.addEventListener("click", function (e) {
    if(e.target.id == "createRoomButton") {
        ...
        document.getElementById("roomname").value = randomString(5, '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ');
    }
            ...
            if(localStorage.getItem("videoPath") == null || localStorage.getItem("videoSize") == null || document.getElementById("join-username").value.length == 0) {
                ...
                .then( async (result) => {
                    ...
                    if(resp.message != "success") {
                        ...
                        document.title = `Party | ${resp.roomName}`
                        roomPage.style.display = "block"
                    }   
                })
                .catch(error => console.log('error', error));
